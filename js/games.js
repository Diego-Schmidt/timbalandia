...
/**
* Crea un generador de secuencia desde hasta
*/
...
function operacionApuesta(){
    let creditoFinal = document.getElementById('balance');
    localStorage.setItem('TC', creditoFinal);
    console.log(creditoFinal);
    // localStorage.setItem('TC', `${resultado}`);
  }
...
function notEnougTC(){
    ...
    Swal.fire({
        title: 'Error!',
        text: '¡No tienes suficiente crédito! compra más TC',
        icon: 'error',
...
// Descontar apuesta del total de TC
function descontarApuesta(){
    let apuesta = document.getElementById('bet');
    let creditos = +localStorage.getItem('TC');
    let resultado = creditos - apuesta;
    localStorage.setItem('TC', `${resultado}`);
  }
